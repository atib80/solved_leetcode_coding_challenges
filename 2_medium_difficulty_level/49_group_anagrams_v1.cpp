...
class Solution {
 public:
  vector<vector<string>> groupAnagrams(vector<string>& strs) {
    ...
    vector<int> visited_words(words_size);

    while (true) {
      vector<string> anagram_words{};
      size_t i{};
      while (i < words_size) {
        if (!visited_words[i]) {
          anagram_words.emplace_back(move(strs[i]));
          visited_words[i] = 1;
          i++;
          break;
        }
      ...
      for (; i < words_size; i++) {
        ...
        if (is_permutation(begin(anagram_words.front()),
                           end(anagram_words.front()), begin(strs[i]),
                           end(strs[i]))) {
          anagram_words.emplace_back(move(strs[i]));
          visited_words[i] = 1;
        }
      ...
      output.emplace_back(move(anagram_words));
    }
...
int main() {
  ...
  // cout << "s.groupAnagrams(input) -> " << result << '\n';
