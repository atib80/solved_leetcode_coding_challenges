...
class Solution {
  static size_t find_minimum_edit_distance(const string& word1,
    ...
    vector<int> word1_missing_characters(256, 0);
    ...
    longest_substr.reserve(dst_last);

    while (dst_first < dst_last) {
      if (word1_missing_characters[word2[dst_first]]) {
        ++dst_first;
        continue;
      }
      ...
      while (dst_offset < dst_last) {
        ...
        if (string::npos != found) {
          ...
          longest_substr.push_back(word2[dst_offset]);
          seq_len++;
          src_offset = found + 1;

        } else {
          if (!src_offset)
            word1_missing_characters[word2[dst_offset]] = 1;
          break;
        }
